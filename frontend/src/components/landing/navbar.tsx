 ...
 return (
  <header
   className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
    scrolled
     ? "bg-surface-0 border-b border-border-subtle shadow-lg py-3"
     : "bg-transparent py-5"
   }`}
  >
   <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
    ...
    <Link href="/" className="flex items-center gap-2.5 group">
     <Image
      ...
      priority
     />
     <span className="text-[10px] font-mono bg-brand-500/15 text-brand-300 px-1.5 py-0.5 rounded border border-brand-500/30 hidden sm:inline-block">
      Soroban v2.1
     </span>
    </Link>

    {/* Desktop Nav Links */}
    <nav className="hidden md:flex items-center gap-1 bg-surface-1 border border-border-subtle rounded-full px-4 py-1.5 ">
     {navLinks.map((link) => (
      <a
       key={link.href}
       href={link.href}
