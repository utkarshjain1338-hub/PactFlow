...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        final String bucketKey = resolveBucketPrefix(isForgotPassword, isAuth, isMutation) + ip;
        final Bucket bucket = cache.computeIfAbsent(bucketKey,
                key -> createBucket(isForgotPassword, isAuth, isMutation));
        final ConsumptionProbe probe = bucket.tryConsumeAndReturnRemaining(1);

        final long limit = resolveLimit(isForgotPassword, isAuth, isMutation);
        ...
        if (probe.isConsumed()) {
            ...
            writeRateLimitExceededResponse(response, path, ip, probe);
        }
    }

    private String resolveBucketPrefix(final boolean isForgotPassword, final boolean isAuth,
    ...
    private long resolveLimit(final boolean isForgotPassword, final boolean isAuth, final boolean isMutation) {
        if (isForgotPassword) {
            return properties.getSecurity().getRateLimit().getForgotPasswordRph();
        } else if (isAuth) {
            return properties.getSecurity().getRateLimit().getAuthEndpointsRpm();
        } else if (isMutation) {
            return properties.getSecurity().getRateLimit().getMutationEndpointsRpm();
        }
        return properties.getSecurity().getRateLimit().getGlobalAuthenticatedRpm();
    }

    private void writeRateLimitExceededResponse(final HttpServletResponse response, final String path,
                                                final String ip, final ConsumptionProbe probe) throws IOException {
